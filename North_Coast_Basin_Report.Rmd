...
output:
  html_document:
    css: Y:\NPS_Annual_Reports\2017\Basin_Reports_2017\DEQTemplate.css
    fig_caption: true
    mode: selfcontained
    ...
    toc_float: true
  word_document:
    reference_docx: \\deqhq1\WQNPS\NPS_Annual_Reports\2017\Basin_Reports_2017\BasinReportTemplate.docx
    toc: false
    fig_caption: true
  pdf_document:
    fig_caption: true
...
nlcd_huc8_df <- read.csv("lookup_tables/NLCD_2011_LU_sqm_by_HUC8.txt", quote="", stringsAsFactors=FALSE)
...
if(nrow(deq319.df) > 0){
  print(knitr::kable(deq319.df, padding = 2, digits = 1, 
...
```{r CWsRF_projects tbl}

pix.df <- pix.df[NULL,]

if(nrow(deqCWSRF.df) > 0){
  print(knitr::kable(deqCWSRF.df, padding = 2, digits = 1, 
...
if(nrow(deqDWSRF.df) > 0){
  print(knitr::kable(deqDWSRF.df, padding = 2, digits = 1, 
...
if(nrow(deqDWPP.df ) > 0){
  print(knitr::kable(deqDWPP.df,  padding = 2, digits = 1, 
...
if(nrow(oweb.projects.df) > 0){
  knitr::kable(oweb.projects.df, padding = 2, digits = 1, row.names=FALSE, 
               caption = tbls(name="tbl.oweb_projects", 
                              caption="OWEB grant funded projects compleated in 2016, the most recent year data is availiable in OWEB's OWRI database."))
}
```
...
knitr::kable(tmdl.imp.df, padding = 2, digits = 1, row.names=FALSE, caption = tbls(name="tbl.dma_projects", caption="TMDL implementaiton activities in 2017 by Designated Managment Agencies or third parties."))
